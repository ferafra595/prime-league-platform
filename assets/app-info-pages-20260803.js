...
function loadInfoPagesStyle(){
  ...
  const link=document.createElement('link');
  link.rel='stylesheet';
  link.href='/assets/info-pages.css?v=20260803-1';
...
async function faqPage(){
  ...
  const groups=[
    {
      title:'Iscrizioni e squadre',
      icon:'📝',
      items:[
        ['Come si iscrive una squadra?','Per partecipare è necessario contattare l’organizzazione e completare la procedura di iscrizione entro la data comunicata per la stagione.'],
        ['Quante squadre partecipano?','La Prime League è strutturata per ospitare indicativamente da 10 a 12 squadre, in base alla stagione.'],
        ['È possibile iscriversi a campionato iniziato?','No. Le iscrizioni delle squadre vengono chiuse prima dell’inizio della competizione.'],
        ['Cosa comprende la quota di iscrizione?','La quota copre i servizi organizzativi previsti per la stagione. Tesseramenti, cauzione ed eventuali costi aggiuntivi vengono comunicati separatamente dall’organizzazione.']
      ]
    },
    {
      title:'Giocatori e tesseramenti',
      icon:'👤',
      items:[
        ['Qual è l’età minima per partecipare?','La partecipazione è consentita a partire dai 17 anni, nel rispetto della documentazione e delle autorizzazioni richieste.'],
        ['Quanti giocatori deve avere una squadra?','Ogni squadra deve presentare una rosa di almeno 12 giocatori. Le condizioni definitive sono indicate nei documenti ufficiali della stagione.'],
        ['Possono partecipare giocatori tesserati con altre società?','Sì, entro il limite massimo previsto dalla Prime League. Attualmente sono ammessi fino a tre tesserati esterni per squadra.'],
        ['È possibile aggiungere giocatori durante la stagione?','Sì, esclusivamente durante la finestra di mercato stabilita dall’organizzazione e completando il relativo tesseramento.'],
        ['Serve il certificato medico?','Ogni partecipante deve essere in possesso della documentazione sanitaria e sportiva richiesta per il tesseramento CSI.']
      ]
    },
    {
      title:'Partite e calendario',
      icon:'⚽',
      items:[
        ['Quanto dura una partita?','Ogni gara ha una durata complessiva di 60 minuti, suddivisa in due tempi.'],
        ['Quando si gioca?','Le gare vengono programmate principalmente il mercoledì, il giovedì e il venerdì, generalmente nelle fasce orarie serali.'],
        ['Una partita può essere spostata?','Sì. In caso di sospensione, rinvio o necessità organizzative, l’Admin può riprogrammare data, ora e campo.'],
        ['Cosa succede se una squadra non si presenta?','La partita può essere assegnata a tavolino con il risultato di 3-0 e può essere applicata una trattenuta sulla cauzione, secondo le regole della stagione.']
      ]
    },
    {
      title:'Classifica e competizioni',
      icon:'🏆',
      items:[
        ['Chi vince il campionato?','La squadra prima classificata al termine della stagione regolare è Campione Prime League e vince la coppa del campionato.'],
        ['Chi partecipa al mini torneo premio?','Le squadre classificate dal secondo al quinto posto. Le semifinali sono 2ª contro 5ª e 3ª contro 4ª.'],
        ['Il mini torneo cambia la classifica?','No. La classifica finale del campionato resta quella determinata dalla stagione regolare.'],
        ['Come si aggiorna la classifica?','Classifica e statistiche vengono aggiornate dopo la pubblicazione del risultato e l’approvazione del referto da parte dell’Admin.']
      ]
    },
    {
      title:'Referti e disciplina',
      icon:'📋',
      items:[
        ['Chi compila il referto?','Le due squadre possono inviare il proprio referto, mentre l’arbitro registra gli eventi disciplinari e le informazioni di propria competenza.'],
        ['Cosa contiene il referto?','Risultato, distinta, convocati, titolari, riserve, presenze, gol, assist, ammonizioni, espulsioni, MVP e note.'],
        ['Chi rende ufficiali i dati?','L’Admin confronta i referti ricevuti, risolve eventuali differenze e approva il documento definitivo.'],
        ['Come funzionano le squalifiche?','Tre ammonizioni comportano una giornata di squalifica. Espulsioni e comportamenti gravi vengono valutati secondo le regole disciplinari della lega.']
      ]
    },
    {
      title:'Votazioni, sponsor e collaborazioni',
      icon:'🤝',
      items:[
        ['Serve registrarsi per votare?','No. Le votazioni pubbliche non richiedono un account, ma ogni dispositivo può esprimere una sola preferenza per ciascun sondaggio.'],
        ['È possibile sponsorizzare una squadra?','Sì. Le aziende possono sostenere una singola squadra oppure diventare sponsor della Prime League.'],
        ['Come posso collaborare con la lega?','La Prime League valuta collaborazioni con arbitri, fotografi, videomaker, speaker, content creator e professionisti del territorio.']
      ]
    }
  ];

  const groupHtml=groups.map((group,gIndex)=>`<section class="faq-group">
    <div class="faq-group-title"><span>${group.icon}</span><h2>${esc(group.title)}</h2></div>
    <div class="faq-list">
      ${group.items.map((item,i)=>`<article class="faq-item ${gIndex===0&&i===0?'open':''}">
        <button type="button" class="faq-question">
          <span>${esc(item[0])}</span><b>+</b>
        </button>
  ...
  set(`${infoHero(
    ...
    'Domande frequenti',
    'Risposte chiare su iscrizioni, giocatori, partite, classifica, referti, premi e collaborazioni.',
    '?'
  )}

  <section class="faq-index">
    <div><strong>Risposte immediate</strong><span>Consulta le categorie oppure apri la domanda che ti interessa.</span></div>
    <a class="btn primary" href="#/contatti">Non trovi la risposta?</a>
  </section>

  <div class="faq-page">${groupHtml}</div>

  <section class="info-final-cta">
    <div><span>Hai ancora dubbi?</span><h2>Parla direttamente con l’organizzazione.</h2></div>
    <a class="btn primary" href="#/contatti">Vai ai contatti</a>
  </section>`,'faq');
...
async function contactsPage(){
  ...
  /*
   * Inserisci qui i recapiti ufficiali quando saranno definitivi.
   * I pulsanti vengono disabilitati automaticamente se il valore è vuoto.
   */
  const contacts={
    whatsapp:'',
    email:'',
  ...
  const action=(href,label,icon)=>{
    if(!href)return `<span class="contact-action disabled">${icon}<span>${esc(label)}</span><small>Recapito in aggiornamento</small></span>`;
    return `<a class="contact-action" href="${esc(href)}" target="_blank" rel="noopener">${icon}<span>${esc(label)}</span><small>Apri contatto</small></a>`;
  };
  ...
  <section class="contact-purpose-grid">
    <article>
      ...
      <p>Vuoi partecipare alla prossima stagione? Contattaci per conoscere disponibilità, quote e documentazione necessaria.</p>
      ${action(contacts.whatsapp?`https://wa.me/${contacts.whatsapp}`:'','Richiedi informazioni','→')}
    </article>

    <article>
      <span>◆</span>
      <h2>Sponsor e aziende</h2>
      <p>Costruiamo proposte dedicate per sponsor della lega, partner ufficiali e aziende interessate a sostenere una squadra.</p>
      ${action(contacts.email?`mailto:${contacts.email}?subject=Richiesta%20sponsorizzazione%20Prime%20League`:'','Diventa sponsor','→')}
    </article>

    <article>
      <span>🎥</span>
      <h2>Collaborazioni</h2>
      <p>Fotografi, videomaker, speaker, arbitri e content creator possono proporre la propria collaborazione alla Prime League.</p>
      ${action(contacts.email?`mailto:${contacts.email}?subject=Collaborazione%20Prime%20League`:'','Collabora con noi','→')}
    </article>
  </section>
  ...
  <section class="info-section contact-direct-section">
    <div class="info-section-heading">
      ...
      <p>I recapiti definitivi potranno essere inseriti in questa pagina senza modificarne la struttura.</p>
    </div>

    <div class="contact-actions-grid">
      ${action(contacts.whatsapp?`https://wa.me/${contacts.whatsapp}`:'','WhatsApp','W')}
      ${action(contacts.phone?`tel:${contacts.phone}`:'','Telefono','☎')}
      ${action(contacts.email?`mailto:${contacts.email}`:'','Email','@')}
      ${action(contacts.instagram,'Instagram','◎')}
    </div>
  </section>
  ...
  <section class="contact-request-guide">
    ...
    <div class="contact-guide-list">
      ...
      <article><b>02</b><span>Squadra o attività rappresentata</span></article>
  ...
  <section class="info-section">
    <div class="info-section-heading">
      ...
      <p>Risultati, contenuti, protagonisti, votazioni e novità della stagione.</p>
    </div>

    <div class="social-contact-grid">
      ${action(contacts.instagram,'Instagram','◎')}
      ${action(contacts.facebook,'Facebook','f')}
      ${action(contacts.tiktok,'TikTok','♪')}
      ${action(contacts.youtube,'YouTube','▶')}
    </div>
  </section>

  <section class="contact-reserved">
    <div><span>Squadre, arbitri e organizzazione</span><h2>Hai già un account?</h2><p>Accedi alla tua area personale per consultare partite, rosa e referti.</p></div>
