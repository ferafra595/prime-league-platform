...
function layout(content, active='home'){
  const nav = [['home','Home'],['partite','Partite'],['classifica','Classifica'],['competizioni','Competizioni'],['squadre','Squadre'],['giocatori','Giocatori'],['statistiche','Statistiche'],['vota','Vota'],['news','News'],['sponsor','Sponsor']];
  const mobile = [
    ['home','⌂','Home'],
    ['partite','⚽','Partite'],
  ...
  return `<div class="shell">
    ...
    <footer class="footer mega-footer"><div class="footer-grid">
      ...
      <div><h4>Prime League</h4><a href="#/statistiche">Statistiche</a><a href="#/vota">Votazioni</a><a href="#/news">Notizie</a><a href="#/sponsor">Sponsor</a></div>
...
async function home(){
  ...
  <section class="section"><div class="section-head"><div><span class="eyebrow">Aggiornamenti</span><h2>Ultime notizie</h2></div><a class="text-link" href="#/news">Tutte le news →</a></div><div class="news-grid">${d.news.slice(0,3).map((n,i)=>`<article class="news-feature ${i===0?'main':''} ${n.cover_url?'has-cover':''}" style="${n.cover_url?`--news-cover:url('${esc(n.cover_url)}')`:''}"><div class="news-overlay"></div><div class="news-content"><span>Prime League</span><h3>${esc(n.title)}</h3><p>${esc(n.excerpt||'')}</p><a href="#/news/${encodeURIComponent(n.slug)}">Leggi la notizia →</a></div></article>`).join('')||'<div class="panel empty">Nessuna notizia.</div>'}</div></section>

  ${d.sponsors.length?`<section class="section sponsors-block"><div class="section-head sponsor-head"><div><span class="eyebrow light">Partner</span><h2>Sponsor ufficiali</h2></div></div><div class="sponsor-wall">${d.sponsors.map(s=>`<a class="sponsor-logo" href="#/sponsor/${encodeURIComponent(s.slug||slugify(s.name))}" aria-label="${esc(s.name)}">${s.logo_url?`<img src="${esc(s.logo_url)}" alt="${esc(s.name)}">`:`<strong>${esc(s.name)}</strong>`}</a>`).join('')}</div></section>`:''}`,'home');
  if(next){
...
async function competitions(seasonId=''){
  ...
  set(`<div class="competitions-page">
    ...
    <section class="future-competitions">
      <div class="competition-section-head"><div><span>Altre competizioni</span><h2>Coppe e tornei</h2><p>Le competizioni aggiuntive create dall’organizzazione per questa stagione.</p></div></div>
      <div class="future-competition-grid">
        ${(d.custom_competitions||[]).map(c=>`<article>
          <span>${c.competition_type==='supercup'?'⭐':c.competition_type==='tournament'?'🥈':'🏆'}</span>
          <h3>${esc(c.name)}</h3>
          <p>${esc(c.description||'Competizione Prime League.')}</p>
          <div class="public-custom-competition-meta">
            <small>${esc(({
              knockout:'Eliminazione diretta',
              groups_knockout:'Gironi + eliminazione',
              round_robin:'Girone',
              single_match:'Partita secca',
              custom:'Formula personalizzata'
            })[c.format]||c.format)}</small>
            ${c.participant_count?`<small>${c.participant_count} partecipanti</small>`:''}
          </div>
          ${c.prize?`<div class="public-custom-prize"><small>Premio</small><strong>${esc(c.prize)}</strong></div>`:''}
          <b>${esc(({
            planned:'Pianificata',
            open:'In preparazione',
            in_progress:'In corso',
            completed:'Conclusa',
            cancelled:'Annullata'
          })[c.status]||c.status)}</b>
        </article>`).join('')||'<div class="competition-bracket-empty"><b>Nessun’altra competizione attiva</b><span>Le nuove coppe e i tornei compariranno qui quando saranno pubblicati dall’organizzazione.</span></div>'}
      </div>
    </section>
  </div>`,'competizioni');
...
${d.sponsors.length?`<section class="section team-section"><div class="section-head"><div><span class="eyebrow">Partner</span><h2>Sponsor della squadra</h2></div></div><div class="team-sponsor-grid">${d.sponsors.map(s=>`<a class="team-sponsor-card" href="#/sponsor/${encodeURIComponent(s.slug||slugify(s.name))}">${s.logo_url?`<img src="${esc(s.logo_url)}" alt="${esc(s.name)}">`:`<strong>${esc(s.name)}</strong>`}</a>`).join('')}</div></section>`:''}`,'squadre');const selector=document.querySelector('#team-season');if(selector)selector.onchange=()=>team(cleanSlug,selector.value)}
...
function loadSponsorHubStyle(){
  if(document.querySelector('link[data-prime-sponsor-hub]'))return;
  const link=document.createElement('link');
  link.rel='stylesheet';
  link.href='/assets/sponsor-hub.css?v=20260820-1';
  link.dataset.primeSponsorHub='1';
  document.head.appendChild(link);
}

function sponsorTierLabel(v){
  return ({main:'Main Partner',official:'Official Partner',partner:'Partner',supporter:'Supporter'})[v]||'Partner';
}

function sponsorLogoBgClass(v){
  return ['dark','prime'].includes(v)?`logo-bg-${v}`:'logo-bg-light';
}
function sponsorPromoVisible(s){
  if(s.promo_is_live===true || Number(s.promo_is_live)===1)return true;
  if(!Number(s.promo_active||0))return false;
  const now=Date.now();
  const parse=(v,end=false)=>{
    if(!v)return null;
    let str=String(v).trim();
    if(/^\d{4}-\d{2}-\d{2}$/.test(str))str+=end?'T23:59:59':'T00:00:00';
    const t=new Date(str).getTime();
    return Number.isFinite(t)?t:null;
  };
  const start=parse(s.promo_start,false), end=parse(s.promo_end,true);
  return (start===null||start<=now) && (end===null||end>=now);
}

async function sponsorHub(){
  loadSponsorHubStyle();
  const d=await api('public/sponsors');
  const sponsors=d.sponsors||[];

  const cards=sponsors.map(s=>`<a class="sponsor-public-card ${s.is_featured?'featured':''}" href="#/sponsor/${encodeURIComponent(s.slug)}">
    <div class="sponsor-public-logo ${sponsorLogoBgClass(s.logo_bg)}">${s.logo_url?`<img src="${esc(s.logo_url)}" alt="${esc(s.name)}">`:`<span>${esc(initials(s.name))}</span>`}</div>
    <div class="sponsor-public-copy">
      <span>${esc(sponsorTierLabel(s.partner_tier))}${s.category?` · ${esc(s.category)}`:''}</span>
      <h3>${esc(s.name)}</h3>
      <p>${esc(s.description||'Scopri il partner e tutti i suoi riferimenti.')}</p>
    </div>
    ${sponsorPromoVisible(s)?`<div class="sponsor-promo-chip">Vantaggio Prime League</div>`:''}
    <b class="sponsor-card-arrow">→</b>
  </a>`).join('');

  set(`<section class="sponsor-hub-hero">
    <div><span>Partner ufficiali</span><h1>Gli sponsor di Prime League</h1><p>Scopri le attività che sostengono il progetto, i loro servizi e gli eventuali vantaggi riservati alla community Prime League.</p></div>
    <div class="sponsor-hub-hero-mark">◆</div>
  </section>

  <section class="sponsor-hub-intro">
    <div><span>Community & territorio</span><h2>Più di un logo.</h2></div>
    <p>Ogni partner può avere una propria vetrina con informazioni, contatti, social, gallery e promozioni dedicate alla community.</p>
  </section>

  <section class="sponsor-public-grid">
    ${cards||'<div class="sponsor-public-empty">I partner della Prime League verranno pubblicati qui.</div>'}
  </section>

  <section class="sponsor-business-cta">
    <div><span>Per le aziende</span><h2>Vuoi diventare partner di Prime League?</h2><p>Scopri le opportunità di sponsorizzazione e porta la tua attività all’interno della community.</p></div>
    <a class="btn primary" href="#/contatti">Contattaci</a>
  </section>`,'sponsor');
}

async function sponsorDetail(slug){
  loadSponsorHubStyle();
  const d=await api(`public/sponsor/${encodeURIComponent(slug)}`);
  const s=d.sponsor;

  const contact=(href,icon,label,value)=>{
    if(!href||!value)return '';
    return `<a class="sponsor-contact-card" href="${esc(href)}" target="_blank" rel="noopener"><span>${icon}</span><div><small>${esc(label)}</small><strong>${esc(value)}</strong></div><b>→</b></a>`;
  };

  const gallery=(s.gallery||[]).filter(Boolean);
  const promo=sponsorPromoVisible(s)?`<section class="sponsor-benefit">
    <div class="sponsor-benefit-badge">VANTAGGIO PRIME LEAGUE</div>
    <div class="sponsor-benefit-main">
      <div><span>Promo riservata</span><h2>${esc(s.promo_title||'Vantaggio dedicato alla community')}</h2><p>${esc(s.promo_description||'')}</p></div>
      ${s.promo_code?`<div class="sponsor-code-box"><small>CODICE PROMO</small><strong>${esc(s.promo_code)}</strong><span>Mostralo all’attività per usufruire della promozione.</span></div>`:''}
    </div>
    ${(s.promo_terms||s.promo_end)?`<div class="sponsor-benefit-terms">${s.promo_terms?`<span>${esc(s.promo_terms)}</span>`:''}${s.promo_end?`<b>Valida fino al ${fmtDate(s.promo_end)}</b>`:''}</div>`:''}
  </section>`:'';

  set(`<section class="sponsor-detail-hero ${s.cover_url?'has-cover':''}" style="${s.cover_url?`--sponsor-cover:url('${esc(s.cover_url)}')`:''}">
    <div class="sponsor-detail-overlay"></div>
    <a class="sponsor-detail-back" href="#/sponsor">← Tutti gli sponsor</a>
    <div class="sponsor-detail-lockup">
      <div class="sponsor-detail-logo ${sponsorLogoBgClass(s.logo_bg)}">${s.logo_url?`<img src="${esc(s.logo_url)}" alt="${esc(s.name)}">`:`<span>${esc(initials(s.name))}</span>`}</div>
      <div><span>${esc(sponsorTierLabel(s.partner_tier))}${s.category?` · ${esc(s.category)}`:''}</span><h1>${esc(s.name)}</h1>${s.team_name?`<small>Partner di ${esc(s.team_name)}</small>`:''}</div>
    </div>
  </section>

  ${promo}

  <section class="sponsor-detail-layout">
    <article class="sponsor-about-card">
      <span>Il partner</span>
      <h2>Scopri ${esc(s.name)}</h2>
      <p>${esc(s.description||'Le informazioni complete dell’attività saranno disponibili presto.')}</p>
      ${s.address?`<div class="sponsor-address"><small>INDIRIZZO</small><strong>${esc(s.address)}</strong></div>`:''}
    </article>

    <aside class="sponsor-quick-actions">
      ${contact(s.whatsapp?`https://wa.me/${String(s.whatsapp).replace(/\D/g,'')}`:'','W','WhatsApp',s.whatsapp)}
      ${contact(s.phone?`tel:${s.phone}`:'','☎','Telefono',s.phone)}
      ${contact(s.email?`mailto:${s.email}`:'','@','Email',s.email)}
      ${contact(s.website_url,'↗','Sito web',s.website_url?'Visita il sito':'')}
      ${contact(s.google_url,'G','Google',s.google_url?'Vedi su Google':'')}
    </aside>
  </section>

  ${(s.instagram_url||s.facebook_url||s.tiktok_url)?`<section class="sponsor-social-section">
    <div><span>Social</span><h2>Segui ${esc(s.name)}</h2></div>
    <div class="sponsor-social-links">
      ${s.instagram_url?`<a href="${esc(s.instagram_url)}" target="_blank" rel="noopener"><b>◎</b><span>Instagram</span></a>`:''}
      ${s.facebook_url?`<a href="${esc(s.facebook_url)}" target="_blank" rel="noopener"><b>f</b><span>Facebook</span></a>`:''}
      ${s.tiktok_url?`<a href="${esc(s.tiktok_url)}" target="_blank" rel="noopener"><b>♪</b><span>TikTok</span></a>`:''}
    </div>
  </section>`:''}

  ${gallery.length?`<section class="sponsor-gallery-section">
    <div class="section-head"><div><span class="eyebrow">Vetrina</span><h2>Gallery</h2></div></div>
    <div class="sponsor-gallery-grid">${gallery.map((img,i)=>`<button class="sponsor-gallery-image" data-src="${esc(img)}"><img src="${esc(img)}" alt="${esc(s.name)} ${i+1}" loading="lazy"></button>`).join('')}</div>
  </section>`:''}

  <section class="sponsor-detail-footer">
    <div><span>Partner Prime League</span><h2>${esc(s.name)}</h2></div>
    <a class="btn" href="#/sponsor">Scopri gli altri partner</a>
  </section>`,'sponsor');

  document.querySelectorAll('.sponsor-gallery-image').forEach(btn=>btn.onclick=()=>{
    const modal=document.createElement('div');modal.className='sponsor-lightbox';
    modal.innerHTML=`<button>×</button><img src="${esc(btn.dataset.src)}" alt="">`;
...
async function news(){
  loading();
  if(!document.querySelector('link[data-prime-news-public]')){
    const link=document.createElement('link');link.rel='stylesheet';link.href='/assets/news-public.css?v=20260829-1';link.dataset.primeNewsPublic='1';document.head.appendChild(link);
  }
  const d=await api('public/news');
  const labels={campionato:'Campionato',risultati:'Risultati',giocatori:'Giocatori',mercato:'Mercato',competizioni:'Competizioni',partner:'Partner',altro:'News'};

  set(`<section class="news-public-hero">
    <div><span>Prime League</span><h1>News</h1><p>Risultati, protagonisti, competizioni, partner e aggiornamenti ufficiali della lega.</p></div>
  </section>

  <section class="news-public-grid">
    ${d.news.map((n,i)=>`<a class="news-public-card ${i===0||n.is_featured?'featured':''} ${n.cover_url?'has-cover':''}" href="#/news/${encodeURIComponent(n.slug)}">
      ${n.cover_url?`<div class="news-public-cover"><img src="${esc(n.cover_url)}" alt="${esc(n.title)}"></div>`:''}
      <div class="news-public-copy">
        <div class="news-public-meta"><span>${esc(labels[n.category]||'News')}</span><small>${fmtDate(n.published_at)}</small></div>
        <h2>${esc(n.title)}</h2>
        <p>${esc(n.excerpt||'')}</p>
        <div class="news-read-more">Leggi l’articolo <b>→</b></div>
      </div>
    </a>`).join('')||'<div class="card empty">Nessuna news pubblicata.</div>'}
  </section>`,'news')
}
...
async function newsArticle(slug){
  ...
  const paragraphs=String(n.body||'').split(/\n{2,}/).map(p=>p.trim()).filter(Boolean);

  set(`<article class="news-article">
    <a class="news-article-back" href="#/news">← Torna alle news</a>

    <header class="news-article-header">
      <div class="news-article-meta"><span>${esc(labels[n.category]||'News')}</span><small>${fmtDate(n.published_at)}</small></div>
      <h1>${esc(n.title)}</h1>
      ${n.excerpt?`<p class="news-article-lead">${esc(n.excerpt)}</p>`:''}
    </header>

    ${n.cover_url?`<div class="news-article-cover"><img src="${esc(n.cover_url)}" alt="${esc(n.title)}"></div>`:''}

    <div class="news-article-body">
      ${paragraphs.map((p,i)=>`<p class="${i===0?'first':''}">${esc(p).replace(/\n/g,'<br>')}</p>`).join('')}
    </div>

    <footer class="news-article-footer">
      <div><span>Prime League</span><strong>Il calcio del territorio, in una nuova dimensione.</strong></div>
      <a class="btn" href="#/news">Altre news</a>
    </footer>
  </article>

  ${(d.related||[]).length?`<section class="news-related">
    <div class="section-head"><div><span class="eyebrow">Continua a leggere</span><h2>Altre notizie</h2></div></div>
    <div class="news-related-grid">
      ${(d.related||[]).map(r=>`<a href="#/news/${encodeURIComponent(r.slug)}">
        ${r.cover_url?`<img src="${esc(r.cover_url)}" alt="${esc(r.title)}">`:''}
        <span>${esc(labels[r.category]||'News')}</span>
        <h3>${esc(r.title)}</h3>
        <p>${esc(r.excerpt||'')}</p>
      </a>`).join('')}
    </div>
...
function dashLayout(body,section='overview'){
  ...
  const teamNav=[['overview','Panoramica'],['profile','Profilo squadra'],['players','Rosa'],['matches','Partite e referti']];
...
async function manageCompetitions(seasonId=''){
  ...
  if(!document.querySelector('link[data-prime-competitions-crud]')){
    const link=document.createElement('link');link.rel='stylesheet';link.href='/assets/competitions-admin-crud.css?v=20260828-1';link.dataset.primeCompetitionsCrud='1';document.head.appendChild(link);
  }
  ...
  const custom=d.custom_competitions||[];
  ...
  const typeLabels={
    ...
    supercup:'Supercoppa',
    tournament:'Torneo',
    friendly:'Evento / Amichevole',
    other:'Altra competizione'
  };
  const formatLabels={
    knockout:'Eliminazione diretta',
    groups_knockout:'Gironi + eliminazione',
  ...
  const statusLabels={
    planned:'Pianificata',
    open:'Iscrizioni / preparazione',
    in_progress:'In corso',
    completed:'Conclusa',
    cancelled:'Annullata'
  };

  const customCards=custom.map(c=>`<article class="admin-custom-competition-card ${c.is_public?'':'is-hidden'}">
    <div class="admin-custom-competition-head">
      <div class="admin-custom-competition-icon">${c.competition_type==='supercup'?'⭐':c.competition_type==='tournament'?'🥈':'🏆'}</div>
      <div>
        <span>${esc(typeLabels[c.competition_type]||'Competizione')}</span>
        <h3>${esc(c.name)}</h3>
        <small>${esc(formatLabels[c.format]||c.format)}</small>
      </div>
      <b class="admin-custom-status status-${esc(c.status)}">${esc(statusLabels[c.status]||c.status)}</b>
    </div>

    ${c.description?`<p>${esc(c.description)}</p>`:''}

    <div class="admin-custom-competition-meta">
      <div><span>Partecipanti</span><b>${c.participant_count||'—'}</b></div>
      <div><span>Inizio</span><b>${c.start_date?fmtDate(c.start_date):'—'}</b></div>
      <div><span>Fine</span><b>${c.end_date?fmtDate(c.end_date):'—'}</b></div>
      <div><span>Pubblica</span><b>${c.is_public?'Sì':'No'}</b></div>
    </div>

    ${c.prize?`<div class="admin-custom-prize"><span>Premio</span><strong>${esc(c.prize)}</strong></div>`:''}

    <div class="admin-row-actions">
      <button class="btn small edit-custom-competition" data-id="${c.id}">Modifica</button>
      <button class="btn small danger delete-custom-competition" data-id="${c.id}">Elimina</button>
    </div>
  </article>`).join('');

  set(dashLayout(`<div class="admin-page-head">
    <div><span class="eyebrow">Struttura sportiva</span><h2>Competizioni</h2><p>Controlla campionato, mini torneo e crea nuove coppe o tornei da un unico pannello.</p></div>
    <div class="admin-competition-head-actions">
      <select class="input admin-competition-season" id="admin-competition-season">${(d.seasons||[]).map(s=>`<option value="${s.id}" ${Number(s.id)===Number(d.season.id)?'selected':''}>${esc(s.name)}</option>`).join('')}</select>
      <button class="btn primary" id="new-custom-competition">Nuova competizione</button>
    </div>
  </div>

    <div id="competition-editor"></div>
    ...
    <section class="admin-custom-competitions-section">
      <div class="admin-custom-section-head">
        <div><span class="eyebrow">Coppe e tornei</span><h2>Altre competizioni</h2><p>Crea e gestisci competizioni aggiuntive per la stagione selezionata.</p></div>
        <b>${custom.length}</b>
      </div>
      <div class="admin-custom-competitions-grid">
        ${customCards||`<button class="admin-create-competition-empty" id="new-custom-competition-empty"><span>＋</span><strong>Crea la prima competizione</strong><small>Coppa, Supercoppa, torneo o formula personalizzata.</small></button>`}
      </div>
  ...
  const seasonOptions=(selected)=> (d.seasons||[]).map(s=>`<option value="${s.id}" ${Number(s.id)===Number(selected)?'selected':''}>${esc(s.name)}</option>`).join('');

  const openEditor=(c={season_id:d.season.id,competition_type:'cup',format:'knockout',status:'planned',is_public:true,sort_order:100})=>{
    document.querySelector('#competition-editor').innerHTML=`<div class="admin-editor-card competition-crud-editor">
      <div class="admin-editor-head">
        <div><span class="eyebrow">${c.id?'Modifica':'Nuova'}</span><h3>${c.id?'Modifica competizione':'Crea competizione'}</h3></div>
        <button type="button" class="admin-close-editor" id="close-competition-editor">×</button>
      </div>

      <form id="custom-competition-form" class="form-grid">
        <div class="field"><label>Stagione</label><select class="input" name="season_id" required>${seasonOptions(c.season_id)}</select></div>
        <div class="field"><label>Ordine</label><input class="input" type="number" name="sort_order" value="${c.sort_order??100}" min="0"></div>

        <div class="field full"><label>Nome competizione</label><input class="input" name="name" value="${esc(c.name||'')}" placeholder="Es. Coppa Prime League" required></div>

        <div class="field"><label>Tipo</label><select class="input" name="competition_type">
          <option value="cup" ${c.competition_type==='cup'?'selected':''}>Coppa</option>
          <option value="supercup" ${c.competition_type==='supercup'?'selected':''}>Supercoppa</option>
          <option value="tournament" ${c.competition_type==='tournament'?'selected':''}>Torneo</option>
          <option value="friendly" ${c.competition_type==='friendly'?'selected':''}>Evento / Amichevole</option>
          <option value="other" ${c.competition_type==='other'?'selected':''}>Altra competizione</option>
        </select></div>

        <div class="field"><label>Formula</label><select class="input" name="format">
          <option value="knockout" ${c.format==='knockout'?'selected':''}>Eliminazione diretta</option>
          <option value="groups_knockout" ${c.format==='groups_knockout'?'selected':''}>Gironi + eliminazione</option>
          <option value="round_robin" ${c.format==='round_robin'?'selected':''}>Girone</option>
          <option value="single_match" ${c.format==='single_match'?'selected':''}>Partita secca</option>
          <option value="custom" ${c.format==='custom'?'selected':''}>Formula personalizzata</option>
        </select></div>

        <div class="field"><label>Stato</label><select class="input" name="status">
          <option value="planned" ${c.status==='planned'?'selected':''}>Pianificata</option>
          <option value="open" ${c.status==='open'?'selected':''}>Preparazione / iscrizioni</option>
          <option value="in_progress" ${c.status==='in_progress'?'selected':''}>In corso</option>
          <option value="completed" ${c.status==='completed'?'selected':''}>Conclusa</option>
          <option value="cancelled" ${c.status==='cancelled'?'selected':''}>Annullata</option>
        </select></div>

        <div class="field"><label>Numero partecipanti</label><input class="input" type="number" name="participant_count" min="2" value="${c.participant_count||''}" placeholder="Es. 8"></div>

        <div class="field"><label>Data inizio</label><input class="input" type="date" name="start_date" value="${esc(c.start_date||'')}"></div>
        <div class="field"><label>Data fine</label><input class="input" type="date" name="end_date" value="${esc(c.end_date||'')}"></div>

        <div class="field full"><label>Descrizione</label><textarea class="input competition-description" name="description" placeholder="Spiega come funziona questa competizione.">${esc(c.description||'')}</textarea></div>
        <div class="field full"><label>Premio</label><input class="input" name="prize" value="${esc(c.prize||'')}" placeholder="Es. Trofeo + premio dedicato"></div>

        <div class="field full"><label class="admin-check"><input type="checkbox" name="is_public" value="1" ${c.is_public!==false?'checked':''}> Visibile nella pagina pubblica Competizioni</label></div>

        <div class="field full"><button class="btn primary">${c.id?'Salva modifiche':'Crea competizione'}</button></div>
      </form>
    </div>`;

    document.querySelector('#close-competition-editor').onclick=()=>document.querySelector('#competition-editor').innerHTML='';

    document.querySelector('#custom-competition-form').onsubmit=async e=>{
      e.preventDefault();
      ...
      payload.is_public=fd.get('is_public')==='1';

      await api(c.id?`admin/competitions/${c.id}`:'admin/competitions',{
        method:c.id?'PUT':'POST',
        body:JSON.stringify(payload)
      });
      manageCompetitions(payload.season_id);
    };

    document.querySelector('#competition-editor').scrollIntoView({behavior:'smooth',block:'start'});
  };

  document.querySelector('#new-custom-competition').onclick=()=>openEditor();
  const emptyButton=document.querySelector('#new-custom-competition-empty');
  if(emptyButton)emptyButton.onclick=()=>openEditor();

  document.querySelectorAll('.edit-custom-competition').forEach(button=>{
    button.onclick=()=>openEditor(custom.find(c=>Number(c.id)===Number(button.dataset.id)));
  });

  document.querySelectorAll('.delete-custom-competition').forEach(button=>{
    button.onclick=async()=>{
      const comp=custom.find(c=>Number(c.id)===Number(button.dataset.id));
      if(confirm(`Eliminare definitivamente "${comp?.name||'questa competizione'}"?`)){
        await api(`admin/competitions/${button.dataset.id}`,{method:'DELETE'});
        manageCompetitions(d.season.id);
      }
    };
  });
}
...
async function manageCalendar(){if(!document.querySelector('link[data-prime-competition-rules]')){const l=document.createElement('link');l.rel='stylesheet';l.href='/assets/competition-rules.css';l.dataset.primeCompetitionRules='1';document.head.appendChild(l)}
  ...
  if(isAdmin){
    ...
    document.querySelector('#generate-calendar').onclick=()=>showForm('editor',`<div class="admin-editor-card">
      <div class="admin-editor-title"><div><span class="eyebrow">Calendario Prime League</span><h3>Genera calendario automatico</h3></div></div>
      <div class="calendar-rule-info"><strong>Distribuzione automatica ufficiale</strong><p>Ogni giornata contiene 5 partite. Il sistema sceglie casualmente 3 giorni tra martedì, mercoledì, giovedì e venerdì e distribuisce le gare con schema <b>2 + 2 + 1</b>.</p><div class="calendar-fixed-rules"><span>📅 Martedì → Venerdì</span><span>⚽ 5 partite a settimana</span><span>2 + 2 + 1</span><span>🕖 19:00 / 20:30</span></div></div>
      <form class="form-grid"><div class="field"><label>Stagione</label><select class="input" name="season_id">${seasonOptions}</select></div><div class="field"><label>Data da cui può iniziare il campionato</label><input class="input" type="date" name="start_date" required></div><div class="field"><label>Data finale indicativa</label><input class="input" type="date" name="end_date"></div><div class="field"><label>Pausa mercato (giorni)</label><input class="input" type="number" min="0" name="market_break_days" value="20"></div><div class="field full"><label>Squadre partecipanti · devono essere 10</label><div class="calendar-team-checks">${state.teams.map(t=>`<label><input type="checkbox" name="team_ids" value="${t.id}" checked> ${esc(t.name)}</label>`).join('')}</div></div><div class="field full"><label>Campo predefinito</label><input class="input" name="venue"></div><div class="field full"><label class="admin-check"><input type="checkbox" name="replace_existing" value="1"> Sostituisci eventuale calendario già presente</label></div><div class="field full"><button class="btn primary" type="submit">Genera andata e ritorno</button></div></form></div>`,async(_,form)=>{const fd=new FormData(form);const teamIds=fd.getAll('team_ids');if(teamIds.length!==10)throw new Error('Per la distribuzione 2 + 2 + 1 devi selezionare esattamente 10 squadre.');const r=await api('admin/calendar/generate',{method:'POST',body:JSON.stringify({season_id:fd.get('season_id'),start_date:fd.get('start_date'),end_date:fd.get('end_date'),market_break_days:fd.get('market_break_days'),venue:fd.get('venue'),replace_existing:fd.get('replace_existing')==='1',team_ids:teamIds})});alert(`Calendario creato: ${r.matches_created} partite. Ogni giornata è stata distribuita casualmente su 3 giorni con schema 2 + 2 + 1.`);manageCalendar()});
...
async function sponsors(){
  if(!['super_admin','organizer'].includes(state.user.role)){location.hash='#/dashboard';return}
  await loadTeams();
  loadSponsorHubStyle();

  if(!document.querySelector('link[data-prime-sponsor-admin]')){
    const link=document.createElement('link');link.rel='stylesheet';link.href='/assets/sponsor-admin.css?v=20260820-1';link.dataset.primeSponsorAdmin='1';document.head.appendChild(link);
  }

  const d=await api('admin/sponsors');
  const sponsors=d.sponsors||[];
  const livePromo=sponsors.filter(s=>s.promo_is_live).length;

  const cards=sponsors.map(s=>`<article class="sponsor-admin-card ${!s.is_active?'is-hidden':''}">
    <div class="sponsor-admin-logo ${sponsorLogoBgClass(s.logo_bg)}">${s.logo_url?`<img src="${esc(s.logo_url)}" alt="${esc(s.name)}">`:`<span>${esc(initials(s.name))}</span>`}</div>
    <div class="sponsor-admin-copy">
      <span>${esc(sponsorTierLabel(s.partner_tier))}${s.category?` · ${esc(s.category)}`:''}</span>
      <h3>${esc(s.name)}</h3>
      <small>${s.level==='team'&&s.team_name?`Squadra: ${esc(s.team_name)}`:'Sponsor della lega'}</small>
      ${s.promo_is_live?'<b class="sponsor-admin-promo">Promo attiva</b>':''}
    </div>
    <div class="sponsor-admin-meta">
      <span class="${s.is_active?'active':'hidden'}">${s.is_active?'Pubblico':'Nascosto'}</span>
      ${s.is_featured?'<b>In evidenza</b>':''}
      <small>Ordine ${s.sort_order||100}</small>
    </div>
    <div class="sponsor-admin-actions">
      <a class="btn small" href="#/sponsor/${encodeURIComponent(s.slug)}" target="_blank">Anteprima</a>
      <button class="btn small edit-sponsor" data-id="${s.id}">Modifica</button>
      <button class="btn small danger delete-sponsor" data-id="${s.id}">Elimina</button>
    </div>
  </article>`).join('');

  set(dashLayout(`<div class="admin-page-head">
    <div><span class="eyebrow">Partner & territorio</span><h2>Sponsor</h2><p>Gestisci vetrine, contatti, social e promozioni dei partner Prime League.</p></div>
    <button class="btn primary" id="new-sponsor">Nuovo sponsor</button>
  </div>

  <section class="sponsor-admin-summary">
    <article><span>Sponsor totali</span><b>${sponsors.length}</b></article>
    <article><span>Pubblici</span><b>${sponsors.filter(s=>s.is_active).length}</b></article>
    <article><span>In evidenza</span><b>${sponsors.filter(s=>s.is_featured).length}</b></article>
    <article><span>Promo attive</span><b>${livePromo}</b></article>
  </section>

  <div id="editor"></div>
  <section class="sponsor-admin-list">${cards||'<div class="sponsor-admin-empty">Nessuno sponsor inserito.</div>'}</section>`,'sponsors'),'');
  bindLogout();

  const uploadBlock=(name,current,label,shape='wide')=>`<div class="sponsor-upload">
    <div class="sponsor-upload-preview ${shape}">${current?`<img src="${esc(current)}" alt="">`:'<span>+</span>'}</div>
    <div><strong>${esc(label)}</strong><small>PNG, JPG o WEBP · massimo 5 MB</small>
      <label class="btn small">Scegli file<input type="file" name="${name}" accept=".png,.jpg,.jpeg,.webp,image/png,image/jpeg,image/webp" hidden></label>
      ${current?`<label class="sponsor-remove-check"><input type="checkbox" name="remove_${name}" value="1"> Rimuovi</label>`:''}
    </div>
  </div>`;

  const form=(s={})=>`<div class="admin-editor-card sponsor-editor">
    <div class="admin-editor-head"><div><span class="eyebrow">${s.id?'Modifica':'Nuovo partner'}</span><h3>${s.id?'Modifica sponsor':'Nuovo sponsor'}</h3></div></div>
    <form class="form-grid" id="sponsor-editor-form">
      <div class="field full sponsor-editor-section-title"><strong>Identità</strong><span>Dati principali mostrati nella pagina sponsor.</span></div>
      <div class="field"><label>Nome attività</label><input class="input" name="name" value="${esc(s.name||'')}" required></div>
      <div class="field"><label>Categoria</label><input class="input" name="category" value="${esc(s.category||'')}" placeholder="Es. Ristorazione, Auto, Benessere"></div>
      <div class="field"><label>Sfondo logo</label><select class="input" name="logo_bg">
        <option value="light" ${(s.logo_bg||'light')==='light'?'selected':''}>Chiaro</option>
        <option value="dark" ${s.logo_bg==='dark'?'selected':''}>Scuro</option>
        <option value="prime" ${s.logo_bg==='prime'?'selected':''}>Blu Prime</option>
      </select><small>Scegli uno sfondo che faccia risaltare il logo.</small></div>
      <div class="field"><label>Livello partner</label><select class="input" name="partner_tier">
        ${[['main','Main Partner'],['official','Official Partner'],['partner','Partner'],['supporter','Supporter']].map(([v,l])=>`<option value="${v}" ${s.partner_tier===v?'selected':''}>${l}</option>`).join('')}
      </select></div>
      <div class="field"><label>Ordine visualizzazione</label><input class="input" type="number" name="sort_order" value="${s.sort_order??100}"></div>
      <div class="field"><label>Associazione</label><select class="input" name="level"><option value="league" ${s.level!=='team'?'selected':''}>Sponsor della lega</option><option value="team" ${s.level==='team'?'selected':''}>Sponsor di una squadra</option></select></div>
      <div class="field"><label>Squadra</label><select class="input" name="team_id"><option value="">Nessuna</option>${state.teams.map(t=>`<option value="${t.id}" ${Number(s.team_id)===Number(t.id)?'selected':''}>${esc(t.name)}</option>`).join('')}</select></div>
      <div class="field full"><label>Descrizione attività</label><textarea class="input sponsor-description" name="description" placeholder="Presenta l'attività, i servizi e ciò che vuoi comunicare nella vetrina.">${esc(s.description||'')}</textarea></div>

      <div class="field full sponsor-editor-media-grid">
        ${uploadBlock('logo_file',s.logo_url||'','Logo','logo')}
        ${uploadBlock('cover_file',s.cover_url||'','Immagine di copertina','wide')}
      </div>

      <div class="field full sponsor-editor-section-title"><strong>Contatti e riferimenti</strong><span>Compila solo i dati forniti dall’attività.</span></div>
      <div class="field"><label>Telefono</label><input class="input" name="phone" value="${esc(s.phone||'')}"></div>
      <div class="field"><label>WhatsApp</label><input class="input" name="whatsapp" value="${esc(s.whatsapp||'')}" placeholder="+39 ..."></div>
      <div class="field"><label>Email</label><input class="input" type="email" name="email" value="${esc(s.email||'')}"></div>
      <div class="field"><label>Sito web</label><input class="input" name="website_url" value="${esc(s.website_url||'')}" placeholder="https://"></div>
      <div class="field full"><label>Indirizzo</label><input class="input" name="address" value="${esc(s.address||'')}"></div>
      <div class="field full"><label>Google / Google Maps</label><input class="input" name="google_url" value="${esc(s.google_url||'')}" placeholder="Link alla scheda Google o a Google Maps"></div>
      <div class="field"><label>Instagram</label><input class="input" name="instagram_url" value="${esc(s.instagram_url||'')}" placeholder="https://"></div>
      <div class="field"><label>Facebook</label><input class="input" name="facebook_url" value="${esc(s.facebook_url||'')}" placeholder="https://"></div>
      <div class="field"><label>TikTok</label><input class="input" name="tiktok_url" value="${esc(s.tiktok_url||'')}" placeholder="https://"></div>

      <div class="field full sponsor-editor-section-title"><strong>Gallery</strong><span>Puoi aggiungere più immagini alla vetrina.</span></div>
      <div class="field full">
        ${(s.gallery||[]).length?`<div class="sponsor-existing-gallery">${s.gallery.map((url,i)=>`<label><img src="${esc(url)}" alt=""><span><input type="checkbox" name="remove_gallery" value="${i}"> Rimuovi</span></label>`).join('')}</div>`:''}
        <label class="sponsor-gallery-upload">+ Aggiungi immagini<input type="file" name="gallery_files" multiple accept=".png,.jpg,.jpeg,.webp,image/png,image/jpeg,image/webp"></label>
      </div>

      <div class="field full sponsor-editor-section-title"><strong>Vantaggio Prime League</strong><span>Facoltativo. La promo sparisce automaticamente quando scade.</span></div>
      <div class="field full"><label class="admin-check"><input type="checkbox" name="promo_active" value="1" ${s.promo_active?'checked':''}> Attiva una promo riservata alla community</label></div>
      <div class="field"><label>Titolo promo</label><input class="input" name="promo_title" value="${esc(s.promo_title||'')}" placeholder="Es. -10% per la community Prime League"></div>
      <div class="field"><label>Codice promo</label><input class="input" name="promo_code" value="${esc(s.promo_code||'')}" placeholder="Es. PRIME10"></div>
      <div class="field full"><label>Descrizione promo</label><textarea class="input" name="promo_description">${esc(s.promo_description||'')}</textarea></div>
      <div class="field full"><label>Condizioni</label><textarea class="input" name="promo_terms" placeholder="Es. Non cumulabile con altre promozioni.">${esc(s.promo_terms||'')}</textarea></div>
      <div class="field"><label>Inizio promo</label><input class="input" type="datetime-local" name="promo_start" value="${s.promo_start?new Date(new Date(s.promo_start).getTime()-new Date(s.promo_start).getTimezoneOffset()*60000).toISOString().slice(0,16):''}"></div>
      <div class="field"><label>Fine promo</label><input class="input" type="datetime-local" name="promo_end" value="${s.promo_end?new Date(new Date(s.promo_end).getTime()-new Date(s.promo_end).getTimezoneOffset()*60000).toISOString().slice(0,16):''}"></div>

      <div class="field full sponsor-editor-section-title"><strong>Pubblicazione</strong></div>
      <div class="field"><label class="admin-check"><input type="checkbox" name="is_featured" value="1" ${s.is_featured?'checked':''}> Sponsor in evidenza</label></div>
      <div class="field"><label class="admin-check"><input type="checkbox" name="is_active" value="1" ${s.is_active!==0?'checked':''}> Visibile sul sito</label></div>
      <div class="field full"><button class="btn primary">${s.id?'Salva tutte le modifiche':'Crea sponsor'}</button></div>
    </form>
  </div>`;

  const open=(s={is_active:1,partner_tier:'partner',sort_order:100,gallery:[]})=>{
    document.querySelector('#editor').innerHTML=form(s);
    const formEl=document.querySelector('#sponsor-editor-form');

    formEl.querySelectorAll('input[type="file"]').forEach(input=>input.onchange=()=>{
      if(input.name==='gallery_files')return;
      const file=input.files?.[0];if(!file)return;
      if(file.size>5*1024*1024){input.value='';return alert('Il file supera 5 MB')}
      const preview=input.closest('.sponsor-upload').querySelector('.sponsor-upload-preview');
    ...
    formEl.onsubmit=async e=>{
      ...
      data.is_featured=fd.get('is_featured')==='1';
      data.is_active=fd.get('is_active')==='1';
      data.promo_active=fd.get('promo_active')==='1';
      data.sort_order=Number(data.sort_order||100);
      data.team_id=data.team_id?Number(data.team_id):null;

      let logo=s.logo_url||'';
      let cover=s.cover_url||'';

      if(fd.get('remove_logo_file')==='1')logo='';
      if(fd.get('remove_cover_file')==='1')cover='';

      const logoFile=formEl.elements.logo_file?.files?.[0];
      const coverFile=formEl.elements.cover_file?.files?.[0];
      if(logoFile)logo=await uploadMediaFile(logoFile,'sponsors',logo,1000);
      if(coverFile)cover=await uploadMediaFile(coverFile,'sponsors',cover,1800);
      data.logo_url=logo;
      data.cover_url=cover;

      let gallery=[...(s.gallery||[])];
      const removeIndexes=fd.getAll('remove_gallery').map(Number).sort((a,b)=>b-a);
      for(const i of removeIndexes)gallery.splice(i,1);

      const files=[...(formEl.elements.gallery_files?.files||[])];
      for(const file of files){
        if(file.size>5*1024*1024)throw new Error(`${file.name}: file superiore a 5 MB`);
        gallery.push(await uploadMediaFile(file,'sponsors','',1600));
      }
      data.gallery=gallery;

      delete data.logo_file;delete data.cover_file;delete data.gallery_files;
      delete data.remove_logo_file;delete data.remove_cover_file;delete data.remove_gallery;

      await api(s.id?`admin/sponsors/${s.id}`:'admin/sponsors',{
        method:s.id?'PUT':'POST',body:JSON.stringify(data)
      });
      sponsors();
    };
  ...
  document.querySelector('#new-sponsor').onclick=()=>open();
  document.querySelectorAll('.edit-sponsor').forEach(b=>b.onclick=()=>open(sponsors.find(s=>Number(s.id)===Number(b.dataset.id))));
  document.querySelectorAll('.delete-sponsor').forEach(b=>b.onclick=async()=>{
    if(confirm('Eliminare definitivamente questo sponsor e la sua vetrina?')){
      await api(`admin/sponsors/${b.dataset.id}`,{method:'DELETE'});
      sponsors();
    }
  });
}
...
async function manageNews(){
  if(!document.querySelector('link[data-prime-news-admin]')){
    const link=document.createElement('link');link.rel='stylesheet';link.href='/assets/news-admin.css?v=20260828-1';link.dataset.primeNewsAdmin='1';document.head.appendChild(link);
  }

  const [d,setup]=await Promise.all([api('admin/news'),api('admin/news/setup-data')]);
  const news=d.news||[];

  const categoryLabels={
    campionato:'Campionato',risultati:'Risultati',giocatori:'Giocatori',
    mercato:'Mercato',competizioni:'Competizioni',partner:'Partner',altro:'Altro'
  };
  const sourceLabels={
    manual:'Manuale',match:'Da partita',round:'Da giornata',
    player:'Da giocatore',competition:'Da competizione',sponsor:'Da sponsor'
  };

  const drafts=news.filter(n=>!Number(n.is_published)).length;
  const auto=news.filter(n=>Number(n.auto_generated)===1).length;
  const published=news.filter(n=>Number(n.is_published)===1).length;

  const cards=news.map(n=>`<article class="news-admin-card ${n.is_published?'published':'draft'}">
    <div class="news-admin-cover">${n.cover_url?`<img src="${esc(n.cover_url)}" alt="">`:'<span>NEWS</span>'}</div>
    <div class="news-admin-content">
      <div class="news-admin-topline">
        <span class="news-status ${n.is_published?'live':'draft'}">${n.is_published?'Pubblicata':'Bozza'}</span>
        <span>${esc(categoryLabels[n.category]||n.category||'Campionato')}</span>
        ${n.auto_generated?'<b>Automatica</b>':''}
      </div>
      <h3>${esc(n.title)}</h3>
      <p>${esc(n.excerpt||'')}</p>
      <small>${esc(sourceLabels[n.source_type]||'Manuale')}${n.is_featured?' · In evidenza':''}</small>
    </div>
    <div class="news-admin-actions">
      <button class="btn small edit-news" data-id="${n.id}">Modifica</button>
      ${!n.is_published?`<button class="btn primary small publish-news" data-id="${n.id}">Pubblica</button>`:''}
      <button class="btn small danger delete-news" data-id="${n.id}">Elimina</button>
    </div>
  </article>`).join('');

  set(dashLayout(`<div class="admin-page-head">
    <div><span class="eyebrow">Redazione Prime League</span><h2>News</h2><p>Il sistema prepara le bozze dai dati della piattaforma. La pubblicazione resta sempre sotto il controllo dell’Admin.</p></div>
    <div class="news-admin-head-actions"><button class="btn" id="generate-news">Crea news da…</button><button class="btn primary" id="new-news">Scrivi manualmente</button></div>
  </div>

  <section class="news-admin-summary">
    <article><span>Bozze</span><b>${drafts}</b><small>da controllare</small></article>
    <article><span>Generate automaticamente</span><b>${auto}</b><small>dal sistema</small></article>
    <article><span>Pubblicate</span><b>${published}</b><small>visibili sul sito</small></article>
    <article><span>Totale</span><b>${news.length}</b><small>contenuti</small></article>
  </section>

  <div class="news-auto-info"><strong>Automazione attiva</strong><span>Quando un referto rende ufficiale una partita, viene creata una bozza risultato. Quando tutte le partite della giornata sono concluse, viene preparato anche il riepilogo della giornata.</span></div>

  <div id="editor"></div>
  <section class="news-admin-list">${cards||'<div class="news-admin-empty">Nessuna news presente.</div>'}</section>`,'news'),'');
  bindLogout();

  const form=(n={category:'campionato'})=>`<div class="admin-editor-card news-editor-card">
    <div class="admin-editor-head"><div><span class="eyebrow">${n.auto_generated?'Bozza automatica':n.id?'Modifica':'Nuova news'}</span><h3>${n.id?'Modifica news':'Scrivi una news'}</h3></div><button type="button" class="admin-close-editor">×</button></div>
    <form class="form-grid">
      <div class="field full"><label>Titolo</label><input class="input" name="title" value="${esc(n.title||'')}" required></div>
      <div class="field"><label>Categoria</label><select class="input" name="category">${Object.entries(categoryLabels).map(([v,l])=>`<option value="${v}" ${n.category===v?'selected':''}>${l}</option>`).join('')}</select></div>
      <div class="field"><label>Stato</label><select class="input" name="is_published"><option value="0" ${!n.is_published?'selected':''}>Bozza</option><option value="1" ${n.is_published?'selected':''}>Pubblicata</option></select></div>
      <div class="field full"><label>Riassunto</label><input class="input" name="excerpt" value="${esc(n.excerpt||'')}"></div>
      ${mediaPicker({name:'cover_file',current:n.cover_url||'',label:'Immagine di copertina',shape:'cover'})}
      <div class="field full"><label>Testo</label><textarea class="input news-body-editor" name="body" required>${esc(n.body||'')}</textarea></div>
      <div class="field full"><label class="admin-check"><input type="checkbox" name="is_featured" value="1" ${n.is_featured?'checked':''}> Metti in evidenza</label></div>
      ${n.auto_generated?`<div class="field full"><div class="news-generated-note">Testo generato automaticamente dai dati ufficiali. Puoi modificarlo liberamente prima della pubblicazione.</div></div>`:''}
      <div class="field full"><button class="btn primary">${n.id?'Salva modifiche':'Salva news'}</button></div>
    </form>
  </div>`;

  const open=n=>{
    showForm('editor',form(n),async f=>{
      f.is_published=f.is_published==='1';
      f.is_featured=f.is_featured==='1';
      await api(n.id?`admin/news/${n.id}`:'admin/news',{
        method:n.id?'PUT':'POST',
        body:JSON.stringify({...f,source_type:n.source_type||'manual',source_id:n.source_id||null,auto_generated:n.auto_generated||0})
      });
      manageNews();
    });
    setTimeout(()=>{const c=document.querySelector('#editor .admin-close-editor');if(c)c.onclick=()=>document.querySelector('#editor').innerHTML=''},0);
  };

  const sourceSelect=(type)=>{
    if(type==='match')return setup.matches.map(x=>`<option value="${x.id}">${esc(x.round_name||'')} · ${esc(x.home_name)} ${x.home_score}-${x.away_score} ${esc(x.away_name)}</option>`).join('');
    if(type==='round')return setup.rounds.map(x=>`<option value="${esc(x.id)}">${esc(x.round_name)}</option>`).join('');
    if(type==='player')return setup.players.map(x=>`<option value="${x.id}">${esc(x.first_name)} ${esc(x.last_name)} · ${esc(x.team_name)}</option>`).join('');
    if(type==='competition')return setup.competitions.map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join('');
    if(type==='sponsor')return setup.sponsors.map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join('');
    return '';
  };

  const openGenerator=()=>{
    document.querySelector('#editor').innerHTML=`<div class="admin-editor-card news-generator-card">
      <div class="admin-editor-head"><div><span class="eyebrow">Automazione</span><h3>Crea news da…</h3></div><button type="button" class="admin-close-editor">×</button></div>
      <form id="news-generator-form" class="form-grid">
        <div class="field"><label>Fonte</label><select class="input" name="source_type" id="news-source-type">
          <option value="match">Partita conclusa</option>
          <option value="round">Giornata completa</option>
          <option value="player">Giocatore</option>
          <option value="competition">Competizione</option>
          <option value="sponsor">Sponsor / Partner</option>
        </select></div>
        <div class="field"><label>Seleziona</label><select class="input" name="source_id" id="news-source-id"></select></div>
        <div class="field full"><div class="news-generator-explain" id="news-generator-explain"></div></div>
        <div class="field full"><button class="btn primary">Genera bozza</button></div>
      </form>
    </div>`;

    const type=document.querySelector('#news-source-type');
    const source=document.querySelector('#news-source-id');
    const explain=document.querySelector('#news-generator-explain');
    const sync=()=>{
      source.innerHTML=sourceSelect(type.value);
      const texts={
        match:'Crea titolo, risultato, marcatori e MVP usando il referto ufficiale.',
        round:'Genera tutti i risultati della giornata e indica la squadra in testa alla classifica.',
        player:'Prepara un articolo dinamico: il taglio cambia in base a gol, assist, MVP, presenze e rendimento disponibile.',
        competition:'Crea una presentazione della competizione con formula, partecipanti e premio.',
        sponsor:'Prepara una news partner includendo l’eventuale Vantaggio Prime League.'
      };
      explain.textContent=texts[type.value]||'';
    };
    type.onchange=sync;sync();
    document.querySelector('#editor .admin-close-editor').onclick=()=>document.querySelector('#editor').innerHTML='';
    document.querySelector('#news-generator-form').onsubmit=async e=>{
      e.preventDefault();
      if(!source.value)return alert('Non ci sono elementi disponibili per questa fonte.');
      await api('admin/news/generate-draft',{method:'POST',body:JSON.stringify({source_type:type.value,source_id:source.value})});
      manageNews();
    };
  };

  document.querySelector('#new-news').onclick=()=>open({category:'campionato',source_type:'manual',auto_generated:0});
  document.querySelector('#generate-news').onclick=openGenerator;
  document.querySelectorAll('.edit-news').forEach(b=>b.onclick=()=>open(news.find(x=>Number(x.id)===Number(b.dataset.id))));
  document.querySelectorAll('.publish-news').forEach(b=>b.onclick=async()=>{
    const n=news.find(x=>Number(x.id)===Number(b.dataset.id));
    await api(`admin/news/${n.id}`,{method:'PUT',body:JSON.stringify({...n,is_published:true,is_featured:Boolean(n.is_featured)})});
    manageNews();
  });
  document.querySelectorAll('.delete-news').forEach(b=>b.onclick=async()=>{
    if(confirm('Eliminare questa news?')){
      await api(`admin/news/${b.dataset.id}`,{method:'DELETE'});
      manageNews();
    }
  });
}
