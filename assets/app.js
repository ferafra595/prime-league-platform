...
function voterToken(){
  let token=localStorage.getItem('prime_league_voter_token');
  if(!token){const bytes=new Uint8Array(24);crypto.getRandomValues(bytes);token=[...bytes].map(b=>b.toString(16).padStart(2,'0')).join('');localStorage.setItem('prime_league_voter_token',token)}
  return token;
}
async function polls(){
  loading();
  if(!document.querySelector('link[data-prime-votes]')){const l=document.createElement('link');l.rel='stylesheet';l.href='/assets/votes.css';l.dataset.primeVotes='1';document.head.appendChild(l)}
  const token=voterToken();
  const d=await api('public/polls',{headers:{'X-Voter-Token':token}});
  const active=(d.polls||[]).filter(p=>p.is_open),closed=(d.polls||[]).filter(p=>!p.is_open);
  const typeLabel={mvp:'MVP della partita',goal:'Gol della giornata',save:'Parata della giornata',custom:'Sondaggio Prime League'};
  const card=p=>{const total=Number(p.votes_count||0),show=p.user_voted||!p.is_open;return `<article class="public-poll-card"><div class="public-poll-head"><div><span>${esc(typeLabel[p.poll_type]||typeLabel.custom)}</span><h2>${esc(p.title)}</h2></div><span class="poll-state ${p.is_open?'open':'closed'}">${p.is_open?'Aperta':'Conclusa'}</span></div>${p.description?`<p class="public-poll-description">${esc(p.description)}</p>`:''}<div class="vote-options">${(p.options||[]).map(o=>{const v=Number(o.votes||0),pct=total?Math.round(v/total*100):0;return `<button class="vote-option ${Number(p.selected_option_id)===Number(o.id)?'selected':''}" data-option="${o.id}" ${p.user_voted||!p.is_open?'disabled':''}><span class="vote-option-avatar">${esc(initials(o.label))}</span><span class="vote-option-content"><strong>${esc(o.label)}</strong>${show?`<span class="vote-result-bar"><i style="width:${pct}%"></i></span><small>${pct}% · ${v} ${v===1?'voto':'voti'}</small>`:'<small>Tocca per votare</small>'}</span>${Number(p.selected_option_id)===Number(o.id)?'<b class="vote-check">✓</b>':''}</button>`}).join('')}</div><div class="public-poll-footer"><span>${total} ${total===1?'voto':'voti'}</span><span>${p.is_open?`Termina ${fmtDate(p.ends_at)}`:'Risultati finali'}</span></div>${p.user_voted?'<div class="vote-confirmed">Voto registrato. Puoi votare una sola volta.</div>':''}</article>`};
  set(shell(`<section class="votes-hero"><span>La voce del pubblico</span><h1>Vota Prime League</h1><p>Non serve registrarsi. Ogni dispositivo può esprimere un solo voto per ciascuna votazione.</p></section><section class="votes-section"><div class="votes-section-head"><span>Adesso</span><h2>Votazioni aperte</h2></div><div class="polls-grid">${active.map(card).join('')||'<div class="votes-empty">Nessuna votazione aperta.</div>'}</div></section>${closed.length?`<section class="votes-section"><div class="votes-section-head"><span>Archivio</span><h2>Votazioni concluse</h2></div><div class="polls-grid">${closed.map(card).join('')}</div></section>`:''}`,'vota'));
  document.querySelectorAll('.vote-option:not([disabled])').forEach(btn=>btn.onclick=async()=>{try{await api('vote',{method:'POST',headers:{'X-Voter-Token':token},body:JSON.stringify({optionId:Number(btn.dataset.option),voter_token:token})});await polls()}catch(e){alert(e.message)}})
}

async function managePolls(){const d=await api('admin/polls');if(!document.querySelector('link[data-prime-votes]')){const l=document.createElement('link');l.rel='stylesheet';l.href='/assets/votes.css';l.dataset.primeVotes='1';document.head.appendChild(l)}const cards=(d.polls||[]).map(p=>`<article class="admin-poll-card"><div class="admin-poll-card-head"><span class="poll-state ${esc(p.status)}">${p.status==='open'?'Aperta':p.status==='closed'?'Chiusa':'Bozza'}</span><strong>${p.votes_count||0} voti</strong></div><h3>${esc(p.title)}</h3><p>${esc(p.description||'Nessuna descrizione')}</p><div class="admin-poll-options">${(p.options||[]).map(o=>`<div><span>${esc(o.label)}</span><b>${o.votes||0}</b></div>`).join('')}</div><div class="admin-row-actions"><button class="btn small edit-poll" data-id="${p.id}">Modifica</button><button class="btn small danger delete-poll" data-id="${p.id}">Elimina</button></div></article>`).join('');set(dashLayout(`<div class="admin-page-head"><div><span class="eyebrow">Coinvolgimento pubblico</span><h2>Votazioni</h2><p>Crea sondaggi senza obbligare gli utenti alla registrazione.</p></div><button class="btn primary" id="new-poll">Nuova votazione</button></div><section class="admin-vote-summary"><div><span>Votazioni</span><b>${(d.polls||[]).length}</b></div><div><span>Aperte</span><b>${(d.polls||[]).filter(p=>p.status==='open').length}</b></div><div><span>Voti totali</span><b>${(d.polls||[]).reduce((a,p)=>a+Number(p.votes_count||0),0)}</b></div></section><div id="editor"></div><div class="admin-polls-grid">${cards||'<div class="votes-empty">Nessuna votazione.</div>'}</div>`,'polls'),'');bindLogout();const form=(p={})=>`<div class="admin-editor-card"><h3>${p.id?'Modifica votazione':'Nuova votazione'}</h3><form class="form-grid"><div class="field full"><label>Titolo</label><input class="input" name="title" value="${esc(p.title||'')}" required></div><div class="field full"><label>Descrizione</label><textarea class="input" name="description">${esc(p.description||'')}</textarea></div><div class="field"><label>Tipo</label><select class="input" name="poll_type"><option value="mvp">MVP</option><option value="goal">Gol della giornata</option><option value="save">Parata</option><option value="custom">Sondaggio libero</option></select></div><div class="field"><label>Stato</label><select class="input" name="status"><option value="draft">Bozza</option><option value="open" ${p.status==='open'?'selected':''}>Aperta</option><option value="closed" ${p.status==='closed'?'selected':''}>Chiusa</option></select></div><div class="field"><label>Inizio</label><input class="input" type="datetime-local" name="starts_at" value="${esc((p.starts_at||'').slice(0,16))}" required></div><div class="field"><label>Fine</label><input class="input" type="datetime-local" name="ends_at" value="${esc((p.ends_at||'').slice(0,16))}" required></div><div class="field full"><label>Opzioni, una per riga</label><textarea class="input poll-options-text" name="options_text" required>${esc((p.options||[]).map(o=>o.label).join('\n'))}</textarea><small>Modificando una votazione già iniziata, i voti vengono azzerati.</small></div><div class="field full"><button class="btn primary">Salva</button></div></form></div>`;const open=(p={})=>showForm('editor',form(p),async f=>{f.options=f.options_text.split('\n').map(label=>({label:label.trim()})).filter(x=>x.label);delete f.options_text;await api(p.id?`admin/polls/${p.id}`:'admin/polls',{method:p.id?'PUT':'POST',body:JSON.stringify(f)});managePolls()});document.querySelector('#new-poll').onclick=()=>open();document.querySelectorAll('.edit-poll').forEach(b=>b.onclick=()=>open(d.polls.find(x=>Number(x.id)===Number(b.dataset.id))));document.querySelectorAll('.delete-poll').forEach(b=>b.onclick=async()=>{if(confirm('Eliminare questa votazione e tutti i voti?')){await api(`admin/polls/${b.dataset.id}`,{method:'DELETE'});managePolls()}})}
