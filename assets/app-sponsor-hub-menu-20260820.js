...
async function sponsorHub(){
  ...
  const cards=sponsors.map(s=>`<a class="sponsor-public-card ${s.is_featured?'featured':''}" href="#/sponsor/${encodeURIComponent(s.slug)}">
    <div class="sponsor-public-logo">${s.logo_url?`<img src="${esc(s.logo_url)}" alt="${esc(s.name)}">`:`<span>${esc(initials(s.name))}</span>`}</div>
    ...
    ${s.promo_is_live?`<div class="sponsor-promo-chip">Vantaggio Prime League</div>`:''}
...
async function sponsorDetail(slug){
  ...
  const promo=s.promo_is_live?`<section class="sponsor-benefit">
  ...
  set(`<section class="sponsor-detail-hero ${s.cover_url?'has-cover':''}" style="${s.cover_url?`--sponsor-cover:url('${esc(s.cover_url)}')`:''}">
    ...
    <div class="sponsor-detail-lockup">
      <div class="sponsor-detail-logo">${s.logo_url?`<img src="${esc(s.logo_url)}" alt="${esc(s.name)}">`:`<span>${esc(initials(s.name))}</span>`}</div>
...
async function sponsors(){
  ...
  const cards=sponsors.map(s=>`<article class="sponsor-admin-card ${!s.is_active?'is-hidden':''}">
    <div class="sponsor-admin-logo">${s.logo_url?`<img src="${esc(s.logo_url)}" alt="${esc(s.name)}">`:`<span>${esc(initials(s.name))}</span>`}</div>
