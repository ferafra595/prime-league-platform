...
function loadInfoPagesStyle(){
  ...
  const link=document.createElement('link');
  link.rel='stylesheet';
  link.href='/assets/info-pages-contact-fix.css?v=20260803-2';
...
async function contactsPage(){
  ...
  const action=(href,label,icon)=>{
    const content=`<span class="contact-action-icon" aria-hidden="true">${icon}</span><span class="contact-action-label">${esc(label)}</span><small class="contact-action-note">${href?'Apri contatto':'Recapito in aggiornamento'}</small>`;
