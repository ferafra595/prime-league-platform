...
async function news(){
  ...
  if(!document.querySelector('link[data-prime-news-public]')){
    const link=document.createElement('link');link.rel='stylesheet';link.href='/assets/news-public.css?v=20260828-1';link.dataset.primeNewsPublic='1';document.head.appendChild(link);
  }
  ...
  set(`<section class="news-public-hero"><div><span>Prime League</span><h1>News</h1><p>Risultati, protagonisti, competizioni, partner e aggiornamenti ufficiali della lega.</p></div></section>
  <section class="news-public-grid">${d.news.map((n,i)=>`<article class="news-public-card ${i===0||n.is_featured?'featured':''} ${n.cover_url?'has-cover':''}">
    ${n.cover_url?`<div class="news-public-cover"><img src="${esc(n.cover_url)}" alt="${esc(n.title)}"></div>`:''}
    <div class="news-public-copy"><div class="news-public-meta"><span>${esc(labels[n.category]||'News')}</span><small>${fmtDate(n.published_at)}</small></div><h2>${esc(n.title)}</h2><p>${esc(n.excerpt||'')}</p><div class="news-public-body">${esc(n.body||'').replace(/\n/g,'<br>')}</div></div>
  </article>`).join('')||'<div class="card empty">Nessuna news pubblicata.</div>'}</section>`,'news')
}
